...
//	private static Logger logger = Logger.getLogger(AntNetCrankControlPlane.class.getName());
	...
	public AntFON(Config aConfig, Accounting aAccounting) {
		...
		for (String id: graph.nodes()) {
			...
			AntRoutingTable art = new AntRoutingTable(id,confidenceLevel,firstWeight,secondWeight,amplifier,correction, powerFactor, deterministic);
			...
			AntFlexiNode colony = new AntFlexiNode(id,Node.Type.FIXED_ALTERNATE,art,antLinks,graph,rerouting,maxReroutingAttempts,reroutingAttempts,sa,model,usageTable);
			nodes.put(id,colony);
		}
	}
	 ...
	 */
	public static String select(RoutingTableEntry neighborhood, LinkedHashMap<String,FlexiLink> links, Ant ant, double alpha, double powerFactor){
